...
def bin_calc_information(inputdata, layerdata, num_of_bins, start, end):
    p_xs, unique_inverse_x = get_unique_probs(inputdata)

    bins = np.linspace(start, end, num_of_bins, dtype='float32') 
    digitized = bins[np.digitize(np.squeeze(layerdata.reshape(1, -1)), bins.squeeze()) - 1].reshape(len(layerdata), -1)
    ...
    for xval in np.unique(unique_inverse_x):
        ...
        H_LAYER_GIVEN_INPUT += - p_xs[int(xval)] * np.sum(p_t_given_x * np.log(p_t_given_x))
